export default function EnglistScript() {
  return (
    <>
      <div>
        <p className="text-2xl lg:text-3xl text-gold-dark text-center font-bold">
          Happy Wedding
        </p>
      </div>
      <div className="text-xl lg:text-2xl text-gold-dark flex items-center justify-center gap-10 mt-6 mb-4">
        <span className="text-center">
          Mr.<span className="font-bold"> Phon Sophat</span>
        </span>
        <span className="text-center">
          Mrs.<span className="font-bold"> Kong Chinda</span>
        </span>
      </div>
      <p className="text-lg lg:text-xl text-gold-dark text-center font-bold">alongside</p>
      <div className="text-xl lg:text-2xl text-gold-dark flex items-center justify-center gap-10 mt-4 mb-4">
        <span className="text-center">
          Mr.<span className="font-bold"> Lang Vuthy</span>
        </span>
        <span className="text-center">
          Mrs.<span className="font-bold"> Ou Thyda</span>
        </span>
      </div>
      <p className="text-lg lg:text-xl text-gold-dark mt-8 text-center leading-9">
        Cordially invite and request the pleasure of your presence at the
        wedding of our beloved children.
      </p>
      <div className="text-xl lg:text-2xl text-gold-dark grid grid-cols-3 place-items-center gap-3 mt-6 mb-4">
        <span className="font-bold text-center">Phon Longdy</span>
        <span className="font-bold justify-self-center">&</span>
        <span className="font-bold text-center">Lang Sreypov</span>
      </div>
      <p className="text-lg lg:text-xl text-gold-dark mt-8 text-center leading-9">
        which will be held from <span className="font-bold">4pm</span> on
      </p>
      <div className="text-lg lg:text-xl text-gold-dark font-bold justify-items-center grid grid-cols-3 gap-3 mt-6">
        <span className="justify-self-end">April</span>
        <span className="justify-self-center">| 22nd |</span>
        <span className="justify-self-start">2026</span>
      </div>
    </>
  );
}
